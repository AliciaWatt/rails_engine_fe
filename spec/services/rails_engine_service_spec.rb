...
RSpec.describe "rails_engine_service" do
  describe "get_merchants" do
    ...
    it "returns the correct data format" do
      ...
      expect(merchant_data[:data][0]).to be_a(Hash)

      expect(merchant_data[:data][0]).to have_key(:id)
      expect(merchant_data[:data][0][:id]).to be_a(String)

      expect(merchant_data[:data][0]).to have_key(:attributes)
      expect(merchant_data[:data][0][:attributes]).to be_a(Hash)

      expect(merchant_data[:data][0][:attributes]).to have_key(:name)
      expect(merchant_data[:data][0][:attributes][:name]).to be_a(String)
    end
  end
